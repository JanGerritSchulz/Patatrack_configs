...
OUTPUTCONFIG = "config/%s_GEN_cfg.py" % CONFIGNAME
...
inOutLines = {
    ...
    "    fileName = cms.untracked.string('General_DQMIO.root')," : [
        "    fileName = cms.untracked.string('file:%s/General_DQMIO.root' % OUTDIR),"
    ],
...
endlines = [
    ...
    '    from Validation.RecoTrack.customiseTrackingNtuple import customiseTrackingNtupleHLT',
    '    # call customization',
    '    process = customiseTrackingNtupleHLT(process) ',
    '    process.TFileService.fileName=cms.string(OUTDIR+"/trackingNtuples_%s.root" % CONFIGNAME.rsplit("/", 1)[1])',
    '    if hasattr(process.trackingNtuple, "ignoreMissingPileupSimHits"):',
    '        process.trackingNtuple.ignoreMissingPileupSimHits = True',
]
